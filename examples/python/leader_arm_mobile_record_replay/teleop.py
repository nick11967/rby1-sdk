#!/usr/bin/env python3
"""Leader-arm teleoperation with arrow-key control of a two-wheel mobile base.

This module is also used by record.py so that control and recording run in the
same process and share the same 100 Hz control clock.
"""
...
# Reuse the hardware-tested leader-arm and gripper drivers without modifying
# the original SDK example.
...
class ArrowKeyController:
    ...
    # Terminals normally emit CSI sequences (ESC [ A), but some terminals use
    # application/SS3 sequences (ESC O A). Support both, plus WASD as a
    # terminal-independent fallback.
    _DIRECTIONS = {
    ...
    def _run(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            # Preserve only a possible prefix of a terminal escape sequence.
...
def _build_mobile_command(base_command: np.ndarray, args):
    return (
...
def _send_base_stop(stream, args) -> None:
    if stream is None:
        return
    try:
...
def run(args, record_path: Optional[Path] = None) -> int:
    if args.linear_speed < 0 or args.angular_speed < 0:
        logging.error("Base speeds must be non-negative")
        return 2
    if args.linear_acceleration <= 0 or args.angular_acceleration <= 0:
        logging.error("Base acceleration limits must be positive")
        return 2
    if args.key_timeout <= 0 or args.mobile_hold_time <= 0:
        logging.error("Key timeout and mobile hold time must be positive")
        return 2
    if args.mobile_ramp_time <= 0 or args.mobile_refresh_time <= 0:
        logging.error("Mobile ramp and refresh times must be positive")
        return 2
    if args.mobile_refresh_time >= args.mobile_hold_time:
        logging.error("Mobile refresh time must be shorter than mobile hold time")
        return 2
    ...
    try:
        ...
        # Keep body and mobility on separate component-level streams. Replacing
        # a mobility velocity profile at 100 Hz can repeatedly restart its ramp.
        ...
        keyboard.start()
        logging.info("Controls: ↑/W forward, ↓/S backward, ←/A left, →/D right")
        logging.info("Release/timeout stops the base; Space stops; Q or Ctrl+C exits")
        if recorder is not None:
